...
const TrackShipment = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-blue-100 flex flex-col items-center py-16 px-2 font-sans ">
      <UserNavbar/>
      <h1 className="text-2xl font-light tracking-wide text-black mb-2 text-center mt-8 underline underline-offset-4 font-sans text-center">Easily track your shipments in real-time and stay updated on every movement from origin to destination.</h1>
      <div className="w-full max-w-xl bg-white rounded-md shadow-md border-2 border-gray-300 p-8 mt-8">
        ...
        <form onSubmit={handleTrack} className="flex flex-col sm:flex-row items-center gap-4 mb-4">
          <div className="flex-1 w-full">
            <input
              type="text"
              className="w-full border-2 border-gray-300 rounded-md px-4 py-3 text-lg focus:outline-none focus:border-blue-500 font-sans"
          ...
          <button
            type="submit"
            className="flex items-center gap-2 bg-blue-500 hover:bg-blue-700 text-white tracking-wide px-6 py-3 rounded-xl transition-all duration-150 shadow font-sans"
            disabled={loading}
          >
        ...
        {shipment && (
          <div className="mt-6">
            <div className="bg-blue-50 rounded-xl p-6 mb-6 border border-blue-100">
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2 mb-2">
                <div className="flex flex-col">
                  ...
                  <span className="text-xl font-light tracking-wide  text-blue-700 font-sans">{shipment.trackingNumber}</span>
                </div>
                <div className="flex flex-col md:items-end">
                  <span className="text-gray-500 text-sm">Status</span>
                  <span className="text-lg font-light tracking-wide text-green-700 font-sans">{shipment.status}</span>
                </div>
              </div>
            ...
            {/* Timeline */}
            <div className="mb-6">
              <h2 className="text-2xl font-light tracking-wide text-blue-700 mb-4 font-sans">Shipment Progress</h2>
                ...
                {shipment.milestones.map((m, idx) => (
                  <div key={m.label} className="flex items-center gap-4">
                    ...
                    <div className="flex-1">
                      <span className={`text-lg font-sans ${m.date ? 'text-blue-900 font-light tracking-wider' : 'text-gray-400'}`}>{m.label}</span>
                      {m.date && <span className="ml-4 text-gray-500 text-sm font-sans">{m.date}</span>}
                    </div>
                  </div>
                ))}
              </div>
